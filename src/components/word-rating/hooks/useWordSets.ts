...
export const useWordSets = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.error('Error saving default languages:', error);
        }
    ...
    const createWordSet = useCallback((name: string, language1?: string, language2?: string, separator?: string, description?: string) => {
    ...
    const getDefaultSet = useCallback((): WordSet => {
        if (wordSets.length === 0) {
            return createWordSet('Default Set', 'English', 'Turkish', ' - ', 'Your first word set');
        }
